...
class DataAnalyzer:
    def __init__(self, api_key: str, versao_modelo: str):
        if not api_key:
            raise ValueError("Erro de Configuração: Chave da API (api_key) ausente.")
            
        try:
            self.client = genai.Client(api_key=api_key)
            self.model_name = f'gemini-{versao_modelo}-flash'
        except Exception as e:
            logging.error(f"Falha ao inicializar o cliente do Google Gemini: {e}")
            raise RuntimeError(f"Erro interno de conexão com a API: {e}")

    def carregar_planilha(self, caminho_arquivo: str) -> pd.DataFrame:
        """Carrega e limpa os dados da planilha, com tratamento contra arquivos corrompidos."""
        try:
            extensao = caminho_arquivo.lower()
            if extensao.endswith(('.csv', '.txt')):
                # Tenta carregar CSV (pode dar erro de encoding, capturado no except)
                df = pd.read_csv(caminho_arquivo)
            elif extensao.endswith(('.xlsx', '.xls')):
                df = pd.read_excel(caminho_arquivo)
            else:
                raise ValueError("Formato de arquivo não suportado. Utilize apenas .csv ou .xlsx.")
                
            if df.empty:
                raise ValueError("A planilha enviada está completamente vazia.")

            # Limpeza vetorizada nativa do Pandas (remove espaços e quebras de linha das colunas)
            df.columns = df.columns.astype(str).str.strip().str.replace('\n', ' ')
            return df
            
        except pd.errors.EmptyDataError:
            raise ValueError("O arquivo enviado não contém dados legíveis.")
        except Exception as e:
            logging.error(f"Erro crítico ao ler o arquivo {caminho_arquivo}: {e}")
            raise RuntimeError(f"Falha ao tentar ler a planilha. Verifique se o arquivo não está corrompido.")

    def _formatar_linha_html(self, linha: str) -> str:
        """Função auxiliar para aplicar os estilos CSS diretos nas tags."""
    ...
    def limpar_markdown_para_html(self, texto_markdown: str) -> str:
        """Converte o Markdown gerado pela IA num HTML elegante e formatado."""
        try:
            ...
            logging.error(f"Erro na conversão de Markdown para HTML: {e}")
            # Em caso de falha na formatação visual, devolve o texto cru para não deixar o sistema cair
            return f"<pre>{texto_markdown}</pre>"
    ...
    def gerar_perfil_pandas(self, df: pd.DataFrame) -> str:
        ...
        try:
            ...
            # Filtra colunas de texto para categorias válidas (entre 2 e 49 itens únicos)
            ...
            if colunas_txt:
                ...
                # Cardápio de Médias Agrupadas (Evita cálculos matemáticos por parte da IA)
                if not colunas_num.empty:
                    perfil.extend([
            ...
            logging.error(f"Erro ao processar as estatísticas do Pandas: {e}")
            return "Resumo estatístico indisponível devido a um erro de formatação nos dados numéricos."

    def analisar_dados(self, df: pd.DataFrame, instrucao_usuario: str, instrucoes_extras: str = "") -> str:
        """Monta o prompt e dispara a requisição protegida para a API do Gemini."""
        try:
            ...
            # Captura falhas de API (Sem internet, limite de requisições, etc)
            logging.critical(f"Erro de comunicação com a API do Gemini: {e}")
            raise RuntimeError("Não foi possível gerar a análise. O serviço da IA pode estar temporariamente indisponível ou ocorreu um erro de comunicação.")